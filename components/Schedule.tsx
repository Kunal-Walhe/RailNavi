...
const Schedule: React.FC<ScheduleProps> = ({ activeStation, trains, isLiveData = false, isLoading = false }) => {
  ...
  const handleTrainClick = async (trainNumber: string) => {
    ...
    try {
      ...
      if (response && response.success && Array.isArray(response.data)) {
        const mappedRoute = response.data.map((stop: any) => ({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      {isLoading && (
        <div className="flex flex-col items-center justify-center p-12 bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-800 shadow-sm animate-pulse">
           <RefreshCw className="text-blue-600 animate-spin mb-4" size={32} />
           <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">Synchronizing Live Fleet Data...</p>
        </div>
      )}

      {!isLoading && !isLiveData && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-4 rounded-lg flex items-center gap-4 shadow-sm animate-in slide-in-from-top-2">
          <div className="p-2 bg-amber-100 dark:bg-amber-900/40 rounded-full text-amber-600">
            <Info size={20} />
          </div>
          <div>
            <p className="text-sm font-bold text-amber-900 dark:text-amber-200 uppercase tracking-tight">Live data unavailable</p>
            <p className="text-xs text-amber-700 dark:text-amber-400 font-medium">Showing fallback schedule from internal cache.</p>
          </div>
        </div>
      )}
            ...
            {filteredTrains.map((train) => (
              <div key={train.id} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg shadow-sm hover:shadow-lg hover:border-blue-300 dark:hover:border-blue-700 transition-all duration-300 overflow-hidden flex flex-col group relative">
                <div className="flex flex-col md:flex-row relative cursor-pointer" onClick={() => handleTrainClick(train.number)}>
                  <div className={`absolute left-0 top-0 bottom-0 w-2 md:w-2 ${train.status === 'ON_TIME' ? 'bg-emerald-500' : train.status === 'DELAYED' ? 'bg-red-500' : train.status === 'BOARDING' ? 'bg-blue-500' : train.status === 'DEPARTED' ? 'bg-gray-400' : train.status === 'RESCHEDULED' ? 'bg-purple-500' : 'bg-amber-500'}`} />
                  ...
                  <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-6 w-full md:border-l border-gray-100 dark:border-slate-800 md:pl-6">
                    ...
                    <div className="hidden md:flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className={`text-xs font-bold ${train.status === 'ON_TIME' ? 'text-emerald-600 dark:text-emerald-400' : train.status === 'DELAYED' ? 'text-red-600 dark:text-red-400' : train.status === 'BOARDING' ? 'text-blue-600 dark:text-blue-400' : train.status === 'DEPARTED' ? 'text-gray-500 dark:text-gray-400' : train.status === 'RESCHEDULED' ? 'text-purple-600 dark:text-purple-400' : 'text-amber-600 dark:text-amber-400'}`}>
                          {train.status === 'ON_TIME' ? t('schedule.on_time') : train.status === 'DELAYED' ? `${t('schedule.delay')} ${train.delayInMinutes}M` : train.status === 'BOARDING' ? 'BOARDING' : train.status === 'DEPARTED' ? 'DEPARTED' : train.status === 'RESCHEDULED' ? 'RESCHEDULED' : 'ARRIVING SOON'}
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    {isLoadingRoute ? (
                      ...
                      <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                        {trainRouteData.map((stop, idx) => {
                          const isAtActiveStation = stop.StationCode === activeStation.code || 
                                                  stop.StationName.toLowerCase().includes(activeStation.city.toLowerCase());
                          return (
                            <div key={idx} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 bg-white dark:bg-slate-900 rounded-lg border transition-all ${isAtActiveStation ? 'border-blue-500 shadow-md ring-1 ring-blue-500/20' : 'border-gray-200 dark:border-slate-800 shadow-sm'}`}>
                              <div>
                                <span className={`text-[10px] font-black px-2 py-1 rounded inline-block mb-1 sm:mb-0 sm:mr-3 border ${isAtActiveStation ? 'bg-blue-600 text-white border-blue-700' : 'text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800'}`}>{stop.StationCode}</span>
                                <span className={`text-xs font-bold ${isAtActiveStation ? 'text-blue-700 dark:text-blue-400' : 'text-gray-800 dark:text-gray-200'}`}>{stop.StationName}</span>
                                {isAtActiveStation && <span className="ml-2 text-[8px] font-black bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 px-1 rounded uppercase tracking-tighter">Current Location</span>}
                              </div>
